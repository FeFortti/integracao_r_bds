...
[Interface de BDs]() consistente para comunicação entre R e SGDBRs
...
`SQLite` precisa de outro argumento: caminho para o BDR (`path = ":dbname:"`).
Porém, a [maioria dos BDRs]() não [está]() em um arquivo, mas sim [em outro]()
[servidor](). Isso faz com que o [código seja semelhante a]():
```{r, eval = FALSE}
con <- DBI::dbConnect(
  RMySQL::MySQL(),
...
Operações [*lazy*]()
  ...
  - [Ao trabalhar com BDRs](), o `dplyr` tenta ser o mais preguiçoso possível
  
  - Atrasa a execução de qualquer trabalho até o último momento possível: 
  reúne tudo o que você deseja fazer e [envia ao BDR em uma única etapa]().
